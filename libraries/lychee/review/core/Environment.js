...
lychee.specify('lychee.Environment').exports(function(lychee, sandbox) {
	...
	sandbox.setProperty('debug', function(assert, expect) {
		...
		assert(this.debug, sandbox.settings.debug);
	...
	sandbox.setProperty('packages', function(assert, expect) {
		...
		assert(this.packages.lychee, sandbox.settings.packages.lychee);
	...
	sandbox.setProperty('sandbox', function(assert, expect) {
		...
		assert(this.sandbox, sandbox.settings.sandbox);
	...
	sandbox.setProperty('tags', function(assert, expect) {
		...
		assert(this.tags.platform, sandbox.settings.tags.platform);
		assert(this.tags.awesome,  sandbox.settings.tags.awesome);
	...
	sandbox.setProperty('target', function(assert, expect) {
		...
		assert(this.target, sandbox.settings.target);
	...
	sandbox.setProperty('timeout', function(assert, expect) {
		...
		assert(this.timeout, sandbox.settings.timeout);
	...
	sandbox.setMethod('load', function(assert, expect) {
		...
		assert(this.setPackages(sandbox.settings.packages), true);
